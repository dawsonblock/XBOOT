...
pub fn load_keyring(path: &Path) -> Result<Keyring> {
    ...
    let data: serde_json::Value = serde_json::from_str(&content)
        .context("invalid keyring JSON")?;
    
    let keys_array = data.get("keys")
    ...
    for (i, key_obj) in keys_array.iter().enumerate() {
        let key_id = key_obj.get("key_id")
            .context(format!("key[{}]: missing 'key_id'", i))?
            .as_str()
            .context(format!("key[{}]: 'key_id' must be string", i))?
            .to_string();
        
        let algorithm = key_obj.get("algorithm")
            .and_then(|v| v.as_str())
            .unwrap_or(SIG_ALGORITHM_ED25519)
            .to_string();
        
        let public_key_b64 = key_obj.get("public_key")
            .context(format!("key[{}]: missing 'public_key'", i))?
            .as_str()
            .context(format!("key[{}]: 'public_key' must be string", i))?;
        
        let public_key = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, public_key_b64)
            .context(format!("key[{}]: invalid base64 in 'public_key'", i))?;
        
        let enabled = key_obj.get("enabled")
            .and_then(|v| v.as_bool())
            .unwrap_or(true);
        
        let description = key_obj.get("description")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
...
pub fn verify_manifest_signature(
    ...
    let signed_fields = manifest.get("manifest_signed_fields")
    ...
    if let Some(keyring) = keyring {
        let key = keyring.get(signer_key_id)
            .context(format!("signer key '{}' not found in keyring", signer_key_id))?;
        ...
        match key.algorithm.as_str() {
            SIG_ALGORITHM_ED25519 => {
                if key.public_key.len() != 32 {
                    bail!("invalid Ed25519 public key length: {}", key.public_key.len());
                }
