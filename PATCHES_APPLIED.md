...
- Rust build/test was not validated in that original container
- live Firecracker/KVM execution was not exercised in that original container
- the pinned internal matrix still needs end-to-end KVM proof as a release gate

## Implemented but not fully re-validated in that pass

- hashed API key verification via `src/auth.rs`
- manifest signing and promotion commands via `src/signing.rs` and `src/main.rs`
- strict startup verification and template quarantine behavior via `src/startup.rs`
