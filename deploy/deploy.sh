...
for server in $SERVERS; do
  ...
  ssh "$server" "cd $REMOTE_ROOT/releases/$RELEASE_ID && sudo $REMOTE_ROOT/releases/$RELEASE_ID/bin/zeroboot template $REMOTE_ROOT/releases/$RELEASE_ID/vmlinux-fc $REMOTE_ROOT/releases/$RELEASE_ID/rootfs-python.ext4 $REMOTE_ROOT/releases/$RELEASE_ID/templates/python 20 /init 512"
  ssh "$server" "cd $REMOTE_ROOT/releases/$RELEASE_ID && sudo $REMOTE_ROOT/releases/$RELEASE_ID/bin/zeroboot template $REMOTE_ROOT/releases/$RELEASE_ID/vmlinux-fc $REMOTE_ROOT/releases/$RELEASE_ID/rootfs-node.ext4 $REMOTE_ROOT/releases/$RELEASE_ID/templates/node 20 /init 512"
  
  # Run smoke test before switching
  echo "Running smoke test..."
  ssh "$server" "sudo $REMOTE_ROOT/releases/$RELEASE_ID/bin/zeroboot test-exec $REMOTE_ROOT/releases/$RELEASE_ID/templates/python python 'print(1+1)'" || {
  ...
  # Update systemd service to use current symlink
  ssh "$server" "sudo sed -i 's|/usr/local/bin/zeroboot|$REMOTE_ROOT/current/bin/zeroboot|g' /etc/systemd/system/zeroboot.service"
